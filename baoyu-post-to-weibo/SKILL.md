...
4. Requires `bun` runtime. If not installed, run: `npm install -g bun`
...
```bash
bun ${SKILL_DIR}/scripts/weibo-post.ts "Hello Weibo!" --image ./photo.png
bun ${SKILL_DIR}/scripts/weibo-post.ts "Watch this" --video ./clip.mp4
```
...
```bash
bun ${SKILL_DIR}/scripts/weibo-article.ts article.md
bun ${SKILL_DIR}/scripts/weibo-article.ts article.md --cover ./cover.jpg
```
