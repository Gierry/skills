...
Options:
  ...
  --provider google|openai|dashscope|replicate|helleai  Force provider (auto-detect by default)
  -m, --model <id>          Model ID
...
function parseArgs(argv: string[]): CliArgs {
  ...
  for (let i = 0; i < argv.length; i++) {
    ...
    if (a === "--provider") {
      const v = argv[++i];
      if (v !== "google" && v !== "openai" && v !== "dashscope" && v !== "replicate" && v !== "helleai") throw new Error(`Invalid provider: ${v}`);
      out.provider = v;
      continue;
    }
...
function detectProvider(args: CliArgs): Provider {
  ...
  const hasHelleai = !!process.env.BAOYU_API_KEY;
  ...
  const available = [hasGoogle && "google", hasOpenai && "openai", hasDashscope && "dashscope", hasReplicate && "replicate", hasHelleai && "helleai"].filter(Boolean) as Provider[];

  if (available.length === 1) return available[0]!;
  if (available.length > 1) return available[0]!;

  throw new Error(
    "No API key found. Set GOOGLE_API_KEY, GEMINI_API_KEY, OPENAI_API_KEY, DASHSCOPE_API_KEY, REPLICATE_API_TOKEN, or BAOYU_API_KEY.\n" +
...
async function loadProviderModule(provider: Provider): Promise<ProviderModule> {
  ...
  if (provider === "helleai") {
    return (await import("./providers/helleai")) as ProviderModule;
  }
