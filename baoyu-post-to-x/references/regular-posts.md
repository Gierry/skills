...
```bash
bun ${SKILL_DIR}/scripts/copy-to-clipboard.ts image /path/to/image.png
```
...
bun ${SKILL_DIR}/scripts/paste-from-clipboard.ts

# Paste to Chrome with retries
bun ${SKILL_DIR}/scripts/paste-from-clipboard.ts --app "Google Chrome" --retries 5

# Quick paste with shorter delay
bun ${SKILL_DIR}/scripts/paste-from-clipboard.ts --delay 200
```
...
Claude:
1. Runs: bun ${SKILL_DIR}/scripts/x-browser.ts "Hello from Claude!" --image ./screenshot.png
...
- **Image paste fails**:
  - Verify clipboard script: `bun ${SKILL_DIR}/scripts/copy-to-clipboard.ts image <path>`
