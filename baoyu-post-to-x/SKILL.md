...
```bash
bun ${SKILL_DIR}/scripts/check-paste-permissions.ts
```
...
```bash
bun ${SKILL_DIR}/scripts/x-video.ts "Check this out!" --video ./clip.mp4
```
...
```bash
bun ${SKILL_DIR}/scripts/x-quote.ts https://x.com/user/status/123 "Great insight!"
```
