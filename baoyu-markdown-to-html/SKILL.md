...
**Agent Execution**: Determine this SKILL.md directory as `SKILL_DIR`. Requires `bun` runtime. If not installed, run: `npm install -g bun`. Replace `${SKILL_DIR}` with actual values.
...
```bash
bun ${SKILL_DIR}/scripts/main.ts <markdown_file> --theme <theme> [--cite]
```
...
```bash
bun ${SKILL_DIR}/scripts/main.ts <markdown_file> [options]
```
...
# With specific theme
bun ${SKILL_DIR}/scripts/main.ts article.md --theme grace

# Theme with custom color
bun ${SKILL_DIR}/scripts/main.ts article.md --theme modern --color red

# Enable bottom citations for ordinary external links
bun ${SKILL_DIR}/scripts/main.ts article.md --cite

# Keep the first heading in content
bun ${SKILL_DIR}/scripts/main.ts article.md --keep-title

# Override title
bun ${SKILL_DIR}/scripts/main.ts article.md --title "My Article"
```
